...
# function ran by another thread that reads dummy values sent by 
# the led process to confirm that the camera is still running
...
if __name__ == "__main__":
    ...
    with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as sock:
        ...
        try:
            ...
            background_init_frames = 0
            
            # initialize background
            while camera.isOpened() and background_init_frames < 100:
                background_init_frames += 1
            ...
            while camera.isOpened():
                ...
                # remove background and crop the input part
                ...
                # if detected object different from background
                if length > 0:
                    maxArea = -1
                    ...
                    # x and y represent the top most point of the detected object
                    x = int(pmax[0] / img.shape[1] * CONFIG.IMAGE_WIDTH)
                    y = int(pmax[1] / img.shape[0] * CONFIG.IMAGE_HEIGHT)

                    # send command to server application
